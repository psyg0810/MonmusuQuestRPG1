...
module CAO::Poker
  ...
  #--------------------------------------------------------------------------
  # ◆ ポーカースキルの効果音　この２つ以外は通常のポーカーと同じ
  #--------------------------------------------------------------------------
  BATTLE_SOUND_WIN     = "mon_fanfale" # 役あり
  BATTLE_SOUND_LOSE    = "Down1"       # 役なし
  #--------------------------------------------------------------------------
  ...
  # ◇ 掛け金の増加量 ( n 倍)
  #--------------------------------------------------------------------------
  DIVIDEND_COVER = [0, 1, 2, 3, 5, 7, 10, 50, 100, 300, 500]
  #--------------------------------------------------------------------------
  ...
  # ◇ ヘルプウィンドウの文章
  #--------------------------------------------------------------------------
  VOCAB_HELP = "\\C[2]<key_a>ボタン\\C[0] : ゲームの説明"
  #--------------------------------------------------------------------------
  # ◇ ゲームの説明
  #--------------------------------------------------------------------------
  MANUAL_LINE = 11            # 行数
...
\C[16]＜賭ける枚数の設定＞\C[0]
 ・\C[2]上下ボタン\C[0]で賭ける枚数を変更
 ・\C[2]<key_c>ボタン\C[0]で賭ける枚数の決定・勝負開始
 ・\C[2]<key_b>ボタン\C[0]で終了

\C[16]＜カード選択＞\C[0]
 ・\C[2]左右ボタン\C[0]でカードを選ぶ
 ・\C[2]<key_c>ボタン\C[0]でカードを裏返す
 ・\C[2]<key_b>ボタン\C[0]で裏向きのカードを交換する
  [EOS]
  #--------------------------------------------------------------------------
  # ◇ 効果音
  #--------------------------------------------------------------------------
...
class Scene_Battle < Scene_Base
  ...
  def process_poker
    ...
    close_skill_name
    process_skill_word(skill)
    display_use_item(@subject.current_action, skill)
...
#==============================================================================
module Sound
  define_poker_sound :play_battlepoker_win,     CAO::Poker::BATTLE_SOUND_WIN
  define_poker_sound :play_battlepoker_lose,    CAO::Poker::BATTLE_SOUND_LOSE
end
