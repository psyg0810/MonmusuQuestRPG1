...
class Game_Interpreter
  ...
  def common_text_to_data
    ...
    Dir::entries(dir_pass).select{|file_name|
      ...
      text_commons = eval(File.read(dir_pass + file_name))
      text_commons.each{|text_common|
        data_common = RPG::CommonEvent.new
        data_common.id = text_common[:id]
        data_common.name = text_common[:name]
        data_common.trigger = text_common[:trigger]
        data_common.switch_id = text_common[:switch_id]
        data_common.list.clear
        text_common[:list].each{|text_command|
          cd = RPG::EventCommand.new
          cd.code = text_command[:code]
          cd.indent = text_command[:indent]
          cd.parameters = text_command[:parameters]
