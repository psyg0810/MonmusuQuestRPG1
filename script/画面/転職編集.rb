...
module Foo::JobChange::EnableCheck
  ...
  def class_change_enable?(id)
    ...
    result.push(need_item?(job))
    result.push(need_class?(job))
    result.push(select_class?(job))
    result.push(different_tribe?(job))
    return experience_class?(job) || result.all?
  end
  #--------------------------------------------------------------------------
  # ● 経験済みクラス？
  #--------------------------------------------------------------------------  
  ...
  #--------------------------------------------------------------------------  
  def need_item?(job)
    return true if job.need_jobchange_item.empty?
    job.need_jobchange_item.each{|item_id|
      return false unless $game_party.has_item?($data_items[item_id])
    }
    return true
  end
  #--------------------------------------------------------------------------
  ...
  def need_class?(job)
    ...
    job.need_jobchange_class.each_with_index{|obj, i|
      return false unless actor.level_list.key?(obj[:id])
      return false if actor.level_list[obj[:id]] < obj[:lv]
    }
    return true
  end
  #--------------------------------------------------------------------------
  ...
  def select_class?(job)
    ...
    job.select_jobchange_class.each_with_index{|obj, i|
      next unless actor.level_list.key?(obj[:id])
      return true if obj[:lv] <= actor.level_list[obj[:id]]
    }
    return false
  end
  #--------------------------------------------------------------------------
...
class Foo::JobChange::Window_Actors < Window_Command
  ...
  def sort(category)
    ...
    method = "sort_by_#{sort_category.to_s}".to_sym
    send(method)
  end
  #--------------------------------------------------------------------------
  # ● ソート（ID順）
...
class Foo::JobChange::Window_ClassStatus < Window_Base
  ...
  def draw_unknown_need_item(y)
    ...
    job.need_jobchange_item.each_with_index{|item_id, i|
      name = $game_party.has_item?($data_items[item_id]) ? $data_items[item_id].name : unknown_name
      draw_text(r, name)
  ...
  def draw_unknown_need_class(y)
    ...
    job.need_jobchange_class.each_with_index{|obj, i|
      r.x = ox + (r.width  * (i % 2))
      r.y = oy + (r.height * (i / 2))
      name = class_change_enable?(obj[:id]) ? $data_classes[obj[:id]].name : unknown_name
      draw_text(r, name)
    }
  ...
  def draw_job_name(y)
    ...
    rect.x += rect.width
    lv = actor.level_list[@class_id]
    lv_s = lv.nil? ? "Lv.-" : "Lv.#{lv}"
    draw_text(rect, lv_s)
  ...
  def draw_job_param(y)
    ...
    rect1 = Rect.new(0, 0, rect_width - 40 - 10, rect_height)
    ...
    job_params.each_with_index{|param, i|
      val_index = status_valuation[i].index{|v| v <= param}
      val_index = -1 if val_index.nil?
      name = valuation_names[val_index]
      rect2.x = rect.x + rect1.width + rect_width  * (i % 3)
      rect2.y = rect.y + rect_height * (i / 3)
      draw_text(rect2, name, 2)
    }
    return rect.y + rect.height
  end
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def valuation_names
    NWConst::JobChange::VALUATION
  end
  #--------------------------------------------------------------------------
  # ● 評価基準値の取得
  #--------------------------------------------------------------------------
  def status_valuation
    NWConst::JobChange::STATUS_VALUATION 
  end
end
...
class Foo::JobChange::Window_ClassName < Window_Command
  ...
  #--------------------------------------------------------------------------
  def select_class_id
    class_id[self.index].nil? ? -1 : class_id[self.index]
  end
  #--------------------------------------------------------------------------
  # ● 表示用クラスIDの取得
  #--------------------------------------------------------------------------
  def class_id
    ...
    return [NWConst::Class::JOB_RANGE, NWConst::Class::TRIBE_RANGE].at(@class_type_id).select{|id|
  ...
  def make_command_list
    ...
    class_name.each{|name|add_command(name, :ok, name != unknown_name)}
  end
  #--------------------------------------------------------------------------
  # ● 項目の描画（オーバーライド）
  #--------------------------------------------------------------------------
  def draw_item(index)
    ...
    change_color(tp_gauge_color2) if actor.class_id == class_id[index]
    change_color(mp_gauge_color2) if actor.tribe_id == class_id[index]
    rect = item_rect_for_text(index)
    rect.width -= 24
    draw_text(rect, command_name(index), alignment)
    
    class_level = actor.level_list[class_id[index]]
    if !class_level.nil? && NWConst::JobChange::CLASS_MASTER_LEVEL <= class_level
      draw_icon(NWConst::JobChange::CLASS_MASTER_ICON_ID, rect.width, rect.y)
    end
  end
end
...
class Scene_JobChange < Scene_MenuBase
  ...
  def create_popup_window
    ...
    @popup_window.set_handler(:actor_ok, method(:process_actor_ok))
    @popup_window.set_handler(:actor_cancel, method(:process_actor_ok_cancel))
  ...
  def create_actors_window
    ...
    @actors_window.set_handler(:ok, method(:process_actor_ok_check))
  end
  #--------------------------------------------------------------------------
  # ● 選択アクターの決定確認
  #--------------------------------------------------------------------------
