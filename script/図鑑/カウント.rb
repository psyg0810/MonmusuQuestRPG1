...
class Game_Party < Game_Unit
  ...
  def gain_item(item, amount, include_equip = false)
    nw_count_gain_item(item, amount, include_equip)
...
class Scene_Battle < Scene_Base
  ...
  def count_up_defeat(subject, target, item)
    return unless target.result.added_states.include?(target.death_state_id)
    if subject.final_elements(item).include?(NWConst::Elem::PLEASURE)
