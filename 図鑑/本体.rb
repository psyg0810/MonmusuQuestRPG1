...
module NWConst::Library
  ...
  # アイテム入手に関する定数
  # 項目名
  ENCOUNTER_ENEMY_PLACE_NAME = "出現場所"
  GET_ITEM_PLACE_NAME = "入手場所"
  GET_ITEM_DROP_NAME  = "落とす敵"
  GET_ITEM_STEAL_NAME = "盗める敵"
  GET_ITEM_ACTOR_NAME = "もらえる仲間"  
  GET_ITEM_NO_NAME    = "なし"
  GET_ITEM_ETC_NAME   = "など"
  # 最大表示サイズ
...
#~         draw_contents_empty
  ...
  def draw_record2(rect)
    ...
    change_color(system_color)
    draw_text(lr, "戦闘回数:");                              lr.y += rect.height
    draw_text(lr, "倒した敵の数:");                          lr.y += rect.height
    draw_text(lr, "倒された回数(ルカ):");                    lr.y += rect.height
    draw_text(lr, "イかされた回数(ルカ):");                  lr.y += rect.height
    draw_text(lr, "仲間にイかされた回数(ルカ):");            lr.y += rect.height
    draw_text(lr, "倒された回数(全員):");                    lr.y += rect.height
    draw_text(lr, "イかされた回数(全員):");                  lr.y += rect.height
    draw_text(lr, "逃げた回数:");                            lr.y += rect.height
    draw_text(lr, "全滅した回数:");                          lr.y += rect.height    
    draw_text(lr, "バトルファック勝利数:");                  lr.y += rect.height
    draw_text(lr, "バトルファック敗北数:");                  lr.y += rect.height
    draw_text(lr, "買い物総額:");                            lr.y += rect.height
    draw_text(lr, "鍛冶利用数:");                            lr.y += rect.height
    draw_text(lr, "転職した回数:");                          lr.y += rect.height
    draw_text(lr, "転種した回数:");                          lr.y += rect.height
    change_color(normal_color)
    draw_text(rr, "#{party_battle}回");                      rr.y += rect.height
    draw_text(rr, "#{party_defeat}回");                      rr.y += rect.height
    draw_text(rr, "#{actor_down(NWConst::Actor::LUCA)}回");  rr.y += rect.height
    draw_text(rr, "#{actor_orgasm(NWConst::Actor::LUCA)}回");rr.y += rect.height
    draw_text(rr, "#{friendly_orgasm}回");                   rr.y += rect.height
    draw_text(rr, "#{party_down}回");                        rr.y += rect.height
    draw_text(rr, "#{party_orgasm}回");                      rr.y += rect.height
    draw_text(rr, "#{party_escape}回");                      rr.y += rect.height
    draw_text(rr, "#{party_lose}回");                        rr.y += rect.height    
    draw_text(rr, "#{battlefuck_win}回");                    rr.y += rect.height
    draw_text(rr, "#{battlefuck_lose}回");                   rr.y += rect.height
    draw_text(rr, "#{purchase_gold}#{Vocab.currency_unit}"); rr.y += rect.height
    draw_text(rr, "#{party_synthesize}回");                  rr.y += rect.height
    draw_text(rr, "#{party_class_change}回");                rr.y += rect.height
    draw_text(rr, "#{party_tribe_change}回");                rr.y += rect.height
  end
  #--------------------------------------------------------------------------
  # ● 周回数
  #--------------------------------------------------------------------------
  ...
  def battlefucker_defeat
    "#{$game_variables[NWConst::Var::BATTLEFUCKER_DEFEAT]}人"
  end
  #--------------------------------------------------------------------------
  # ● 実績
  #--------------------------------------------------------------------------
  ...
  def draw_actor_stat(y, actor)
    ...
    change_color(system_color)
    draw_text(lr, txt)
    txt = "#{$game_library.actor_battle(actor.id).to_i}回"
    change_color(normal_color)
    draw_text(rr, txt)
    lr.y += lr.height
    rr.y += rr.height    
    ...
    change_color(system_color)
    draw_text(lr, txt)
    txt = "#{actor_defeat(actor.id).to_i}体"
    change_color(normal_color)
    draw_text(rr, txt)
    lr.y += lr.height
    rr.y += rr.height
    ...
    change_color(system_color)
    draw_text(lr, txt)
    txt = "#{actor_carry(actor.id).to_i}体"
    change_color(normal_color)
    draw_text(rr, txt)
    lr.y += lr.height
    rr.y += rr.height
    ...
    change_color(system_color)
    draw_text(lr, txt)
    txt = "#{actor_down(actor.id).to_i}回"
    change_color(normal_color)
    draw_text(rr, txt)
    lr.y += lr.height
    rr.y += rr.height
    ...
    change_color(system_color)
    draw_text(lr, txt)
    txt = "#{actor_orgasm(actor.id).to_i}回"
    change_color(normal_color)
    draw_text(rr, txt)
    lr.y += lr.height
    rr.y += rr.height
    ...
    change_color(system_color)
    draw_text(lr, txt)
    txt = "#{$game_library.actor_steal(actor.id).to_i}回"
    change_color(normal_color)
    draw_text(rr, txt)
    lr.y += lr.height
    rr.y += rr.height
  ...
  def draw_actor_illustrator(actor)
    ...
    draw_text(rect, "イラスト:#{actor.illustrator_name}", 2)
  end
  #--------------------------------------------------------------------------
  # ● 敵キャラ
  #--------------------------------------------------------------------------
  ...
  def draw_chara_description(y, chara)
    ...
    # 解説文章の取得
    desc = ["解説無し"]
    desc = ACTOR_DESCRIPTION[@id] if chara.is_a?(RPG::Actor) && ACTOR_DESCRIPTION.key?(@id)
    desc = ENEMY_DESCRIPTION[@id] if chara.is_a?(RPG::Enemy) && ENEMY_DESCRIPTION.key?(@id)
  ...
  def draw_enemy_stat(y, enemy)
    ...
    rr = half_right_rect(rect.y)
    if enemy.follower?
      txt = "仲間になって#{$game_party.exist_all_actor_id?(enemy.follower_actor_id) ? "いる" : "いない"}"
      change_color(normal_color)
      draw_text(lr, txt)
      lr.y += lr.height + LINE_HEIGHT
      rr.y += rr.height + LINE_HEIGHT
...
class Window_Library_EnemyCommand < Window_Command
  ...
  def make_command_list
    ...
    if enemy?
      add_command("敗北回想",   :memory_event, enable && memory_event?)
      add_command("ＣＧ閲覧",   :cg_view, enable && cg_view?)
      ...
      add_command("ＣＧ閲覧",   :actor_cg_view, enable && actor_cg_view?)
      add_command("キャンセル", :cancel, true)
...
class Scene_Library < Scene_MenuBase
  ...
  def terminate
    super
    replay_bgm_and_bgs unless SceneManager.scene_is?(Scene_CGViewer) || SceneManager.scene_is?(Scene_Novel)
  end
  #--------------------------------------------------------------------------
  # ● BGMの演奏
  #--------------------------------------------------------------------------
