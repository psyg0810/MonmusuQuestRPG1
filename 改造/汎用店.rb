...
class Window_ShopCompare < Window_Base
  ...
  def draw_actor_equip_info(x, y, actor)
    ...
    rect.y += line_height    
    draw_item_param_change(current_item, rect.x, rect.y, enabled)
  end
  #--------------------------------------------------------------------------
  # ● アイテムのパラメータ変化値の描画
  #--------------------------------------------------------------------------
  def draw_item_param_change(current_item, x, y, enable)
    if enable
      base_rect = Rect.new(x, y, (contents.width / 2) / 3, 20)
      params = current_item ? current_item.params : [0] * 8
      params6 = (2..7).collect{|i| params[i] - @item.params[i]}
