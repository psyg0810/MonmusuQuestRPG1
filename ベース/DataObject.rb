...
class RPG::UsableItem < RPG::BaseItem
  ...
  #--------------------------------------------------------------------------
  def add_anime
    @data_ex.key?(:add_anime) ? @data_ex[:add_anime] : 0
  end
  #--------------------------------------------------------------------------
  # ● 基本属性配列
  #--------------------------------------------------------------------------
  ...
  def weapon_rate(wtype_id)
    ...
    return @data_ex[:weapon_rate][wtype_id]
  end
  #--------------------------------------------------------------------------
  # ● 薬の知識適用？
  #--------------------------------------------------------------------------  
...
class RPG::Actor < RPG::BaseItem
  ...
  def original_persona_id
    @data_ex.key?(:original_persona_id) ? @data_ex[:original_persona_id] : 0    
  end
  #--------------------------------------------------------------------------
  # ● 誘惑時使用スキル
  #--------------------------------------------------------------------------
...
class RPG::Skill < RPG::UsableItem
  ...
  def nw_note_analyze
    ...
    self.note.each_line do |line|
      if NWRegexp::Skill::HP_COST_EX.match(line)
        ...
        @data_ex[:not_learn_actors] ||= []        
        $1.split(/\,\s?/).each{|id|@data_ex[:not_learn_actors].push(id.to_i)}
  ...
  def not_learn_actors
    @data_ex.key?(:not_learn_actors) ? @data_ex[:not_learn_actors] : []
  end  
  #--------------------------------------------------------------------------
  # ● メモライズコスト
  #--------------------------------------------------------------------------
