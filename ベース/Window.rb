...
class Window_Base < Window
  ...
  def draw_actor_simple_status(actor, x, y, next_exp = false)
    ...
    #
    change_color(system_color)
    draw_text(x + 152, y + contents.font.size * 0 + 5, 22, line_height, Vocab::level_a)
    change_color(normal_color)
    draw_text(x + 170, y + contents.font.size * 0 + 5, 22, line_height, actor.base_level, 2)
    #
    change_color(tp_gauge_color2)
    draw_text(x, y + contents.font.size * 1 + 5, 152, line_height, actor.class.name)
    change_color(system_color)
    draw_text(x + 152, y + contents.font.size * 1 + 5, 22, line_height, Vocab::level_a)
    change_color(normal_color)
    draw_text(x + 170, y + contents.font.size * 1 + 5, 22, line_height, actor.class_level, 2)
    #
    change_color(mp_gauge_color2)
    draw_text(x, y + contents.font.size * 2 + 5, 152, line_height, actor.tribe.name)
    change_color(system_color)
    draw_text(x + 152, y + contents.font.size * 2 + 5, 22, line_height, Vocab::level_a)
    change_color(normal_color)
    draw_text(x + 170, y + contents.font.size * 2 + 5, 22, line_height, actor.tribe_level, 2)
...
class Window_SkillCommand < Window_Command
  ...
  def make_command_list
    ...
    all_skill_types.sort{|a, b|
      [enable_skill_types.include?(a) ? 0 : 1, NWConst::SType::PRIORITY.include?(a) ? 0 : 1, a] <=>
      [enable_skill_types.include?(b) ? 0 : 1, NWConst::SType::PRIORITY.include?(b) ? 0 : 1, b]
    }.each do |stype_id|
      next unless @actor.skills.any?{|skill| skill.stype_id == stype_id}
...
class Window_ItemCost < Window_Base
  ...
  def item_text(item)
    ...
    item.item_cost.each {|cost|
      ...
      c = have >= cost[:num] ? 1 : 0
      texts.push( sprintf("%d%s×%d（所持数：%d）", c, name, cost[:num], have) )
    }
    return texts
  end
  #--------------------------------------------------------------------------
...
class Window_BattleLog < Window_Selectable
  ...
  def replace_ext_character(original_text, subject, targets)
    ...
    text.gsub!(/\\e/i) { targets.first.name + ((2 <= targets.size) ? "達" : "") }
    return text
  end
  #--------------------------------------------------------------------------
  # ○ スキル／アイテム使用の表示
  ...
  def display_stealed(target)
    ...
    if target.result.stealed_item_empty
      ...
      add_text(sprintf(Vocab::Stealed, target.name, sprintf(item_format, target.result.stealed_item_id)))
    end
    wait    
  end
  #--------------------------------------------------------------------------
  ...
  # ● 最終反撃の表示
  #--------------------------------------------------------------------------
  def display_final_invoke(target, item)
    add_text(sprintf(Vocab::FinalInvoke, target.name, item.name))
    wait
    back_one
  end
  #--------------------------------------------------------------------------
...
class Window_BattleStatus < Window_Selectable
  ...
  #--------------------------------------------------------------------------
  def current_item_enabled?
    !(BattleManager.shift_change? && SceneManager.scene.no_change_bind?)
  end
  #--------------------------------------------------------------------------
  # ○ ウィンドウ幅の取得
  #--------------------------------------------------------------------------
...
class Window_BattleEnemy < Window_Selectable
  ...
  def draw_item(index)
    ...
    name = e.name.clone
    name += "(友好度:#{e.friend})" if @friend_draw
